...
public class ConstantsContext {
    ...
    public static String getReleaseVersion() {
        ...
        if (ToolUtil.isEmpty(systemReleaseVersion)) {
            log.error("系统常量存在空值！常量名称：GUNS_SYSTEM_RELEASE_VERSION，采用默认值：guns");
    ...
    public static String getFileUploadPath() {
        ...
        if (ToolUtil.isEmpty(gunsFileUploadPath)) {
            log.error("系统常量存在空值！常量名称：GUNS_FILE_UPLOAD_PATH，采用默认值：系统tmp目录");
            return getTempPath();
        } else {
            //判断有没有结尾符
    ...
    public static String getBpmnFileUploadPath() {
        ...
        if (ToolUtil.isEmpty(bpmnFileUploadPath)) {
            log.error("系统常量存在空值！常量名称：GUNS_BPMN_FILE_UPLOAD_PATH，采用默认值：系统tmp目录");
            return getTempPath();
        } else {
            //判断有没有结尾符
    ...
    public static String getJwtSecret() {
        ...
        if (ToolUtil.isEmpty(systemReleaseVersion)) {
            ...
            log.error("jwt密钥存在空值！常量名称：GUNS_JWT_SECRET，采用默认值：随机字符串->" + randomSecret);
            return randomSecret;
        } else {
            return systemReleaseVersion;
        }
    ...
    public static Long getJwtSecretExpireSec() {
        ...
        if (ToolUtil.isEmpty(systemReleaseVersion)) {
            log.error("jwt密钥存在空值！常量名称：GUNS_JWT_SECRET_EXPIRE，采用默认值：1天");
            ...
            try {
                ...
                log.error("jwt密钥过期时间不是数字！常量名称：GUNS_JWT_SECRET_EXPIRE，采用默认值：1天");
                CONSTNTS_HOLDER.put("GUNS_JWT_SECRET_EXPIRE", String.valueOf(defaultSecs));
                return defaultSecs;
            }
        }
    ...
    public static String getTokenHeaderName() {
        ...
        if (ToolUtil.isEmpty(tokenHeaderName)) {
            ...
            log.error("获取token的header标识为空！常量名称：GUNS_TOKEN_HEADER_NAME，采用默认值：" + defaultName);
            return defaultName;
        } else {
